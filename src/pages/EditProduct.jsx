...
function EditProduct() {
    ...
    useEffect(() => {
        databases.getDocument(dbID, collectionId, prodId)
            .then(response => {
                console.log("productData:", response)
                setValue("prodName", response.name)
                setValue("prodPrice", response.price)
                setValue("prodDescription", response.description)
                setValue("availability", response.availability ? "true" : "false")
                setValue("productCategory", response.category)
            ...
            <div className="flex flex-col items-center mt-20">
                ...
                <form action="" className="w-[400px] product-form">
                    ...
                    <div className="flex flex-col">
                        <label className="ml-2 text-lg" htmlFor="">Category</label>
                        <select className="w-full py-2 pl-3 mt-2 text-black bg-gray-200 rounded-xl" name="" id="" {...register("productCategory")}>
                    ...
                    <div className="my-5">
                        <label className="ml-2 text-lg" htmlFor="product-image">Product preview</label>
                        {currentImage && (
                            <img src={currentImage} alt="Current Product" className="w-[150px] h-auto my-2 rounded-lg shadow-md" />
                        )}
                        <input
                            id="product-image"
                            type="file"
                    ...
                    <div className="text-center mt-7">
                        <button className="px-5 py-2 text-xl text-white bg-green-800 rounded-full" onClick={handleSubmit(onSubmit)}>Edit Product</button>
                    </div>
                </form>
